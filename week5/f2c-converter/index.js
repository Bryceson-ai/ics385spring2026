...
app.get('/f2c', (req, res) => {
  ...
  if (typeof fahrenheit !== 'undefined' && typeof centigrade === 'undefined') {
    ...
    return res.redirect(`/f2c?fahrenheit=${encodeURIComponent(f)}&centigrade=${encodeURIComponent(c)}`);
  }
...
app.post('/f2c', (req, res) => {
  ...
  res.redirect(`/f2c?fahrenheit=${encodeURIComponent(f)}&centigrade=${encodeURIComponent(c)}`);
});
