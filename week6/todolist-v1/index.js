...
let choresItems = ["Do the laundry", "Wash the dishes", "Take out the trash"];
let campingItems = ["Pack tent", "Bring sleeping bag", "Pack flashlight"];

const workListTitle = "Work Items To-Do List";
const choresListTitle = "Chores To-Do List";
const campingListTitle = "Camping To-Do List";
...
app.post("/", function(req, res) {
  ...
  // if list ==== Weekend then go to /weekend
        ...
        if (req.body.list === workListTitle) {
        workItems.push(item);
        res.redirect("/work");
        }
        else if (req.body.list === choresListTitle) {
            choresItems.push(item);
            res.redirect("/chores");
        }
        else if (req.body.list === campingListTitle) {
...
app.get("/work", function(req, res){
    res.render("list", {listTitle: workListTitle, newListItems: workItems})
});

app.get("/chores", function(req, res){
    res.render("list", {listTitle: choresListTitle, newListItems: choresItems});
});

app.get("/camping", function(req, res){
    res.render("list", {listTitle: campingListTitle, newListItems: campingItems});
});
